...
$sth = $dbh->prepare("CREATE TABLE IF NOT EXISTS `sasidhartraps` ( `id` int(11) NOT NULL AUTO_INCREMENT, `STATUS` tinytext NOT NULL, `IP` tinytext NOT NULL, `TIME` tinytext NOT NULL, `PREVIOUS_STATUS` tinytext NOT NULL, `PREVIOUS_TIME` tinytext NOT NULL, `STATUS1` tinytext NOT NULL, `PREVIOUS_STATUS1` tinytext NOT NULL, PRIMARY KEY (`id`) ) ENGINE=InnoDB DEFAULT CHARSET=LATIN1 AUTO_INCREMENT=1");
...
$time=time();
	...
	$sth11 = $dbh->prepare("select * from `sasidhartraps` where `IP`='$ip'");
	$sth11->execute;
	...
	$sth1 = $dbh->prepare("select `STATUS`,`TIME`, `STATUS1` from `sasidhartraps` where `IP`='$ip'");
	$sth1->execute;
	...
	$sth2 = $dbh->prepare("UPDATE `sasidhartraps` SET `STATUS`='$status', `TIME`='$time', `PREVIOUS_STATUS`='$previousStatus', `PREVIOUS_TIME`='$previousTime', `STATUS1`='$status1', `PREVIOUS_STATUS1`='$previousStatus1'  WHERE `IP` = '$ip'");
	$sth2->execute;
	...
	$sth3 = $dbh->prepare("insert into `sasidhartraps` (IP,STATUS,TIME,STATUS1,PREVIOUS_STATUS1) VALUES ('$ip','$status','$time', '$status1','-')");	
	$sth3->execute;
...
$sth4 = $dbh->prepare("select * from `sasidhartrapdevice`");
$sth4->execute;
	...
	($session,$error) = Net::SNMP->session(Hostname => $snmp_credentials[1],Community => $snmp_credentials[3],Port => $snmp_credentials[2]); 
	 ...
	 $sth5 = $dbh->prepare("select * from `sasidhartraps` WHERE `STATUS` = 'FAIL' AND `IP` = '$ipFAIL'");
	 $sth5->execute;
	 ...
	 $sth6 = $dbh->prepare("select * from `sasidhartraps` WHERE `STATUS` = 'DANGER'");
	 $sth6->execute;
   $count = $sth6->rows();
